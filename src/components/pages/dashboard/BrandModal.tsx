...
export default function BrandModal({ open, editingBrand, onClose }: BrandModalProps) {
    ...
    const handleSubmit = async (values: { name: string; description?: string }) => {
        try {
            if (isEditing) {
                ...
                const result = await createBrand(values).unwrap();
                if (result.success) {
            ...
            toast.error(err?.data?.message || `Failed to ${isEditing ? 'update' : 'create'} brand.`);
        }
    };
                ...
                <div className="flex items-center gap-3 pb-2 pt-1 border-b border-gray-100 dark:border-[#303030] mb-4">
                    ...
                    <div>
                        ...
                        <p className="text-xs text-gray-500 dark:text-gray-400 font-normal">
                            {isEditing ? 'Update brand details' : 'Create a new brand for your products'}
                        </p>
                    </div>
                </div>
            }
