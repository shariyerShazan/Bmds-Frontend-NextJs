...
export default function EmployeeManagementTab() {
    ...
    const handleStatusToggle = async (userId: string, active: boolean) => {
        try {
            ...
            const result = await updateUserStatus({ id: userId, status: newStatus }).unwrap();
            if (result.success) {
                    ...
                    <table className="min-w-full divide-y divide-gray-200 dark:divide-[#303030]">
                        <thead className="bg-gray-50 dark:bg-[#1f1f1f] transition-colors">
                            <tr>
                                ...
                                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Contact No</th>
                                    ...
                                    <tr key={user.id} className="hover:bg-gray-50 dark:hover:bg-[#1f1f1f] transition-colors">
                                        <td className="px-6 py-4 whitespace-nowrap">
                                            <div className="flex items-center gap-3">
                                                <div className="w-10 h-10 rounded-full bg-[#272877]/10 flex items-center justify-center shrink-0">
                                                    <UserRound size={18} className="text-[#272877] dark:text-[#6e6fe4]" />
                                                </div>
                                                <div>
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap text-center">
                                            <Popconfirm
                                                ...
                                                onConfirm={() => handleStatusToggle(user.id, user.status !== 'ACTIVE')}
                                                okText="Yes"
                                                cancelText="No"
                                                okButtonProps={{ className: '!bg-[#272877] hover:!bg-[#1e1f5e]' }}
                                            >
                                                <Tooltip title={user.status === 'ACTIVE' ? 'Active (Click to block)' : 'Blocked (Click to unblock)'}>
