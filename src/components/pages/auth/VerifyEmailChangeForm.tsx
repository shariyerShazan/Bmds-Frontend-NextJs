...
import { ArrowLeftOutlined, CheckCircleFilled, CloseCircleFilled, LoadingOutlined } from '@ant-design/icons';
...
export default function VerifyEmailChangeForm() {
    ...
    const verifyEmail = useCallback(async () => {
        if (!token) {
            setStatus('error');
            setErrorMessage('Invalid or missing verification token. Please request a new email change.');
            return;
        }
        ...
        try {
            ...
            setStatus('error');
            setErrorMessage('Failed to verify email change. The link may have expired.');
        }
    }, [token]);
                ...
                {status === 'success' && (
                    <div className="text-center py-8">
                        ...
                        <p className="text-sm text-gray-500 dark:text-gray-400 mb-6 transition-colors">
                            Your email has been changed. Please log in with your new email to continue.
                        </p>
                        <Link
                            href="/auth/login"
