...
import { CalendarOutlined, CheckCircleOutlined, ClockCircleOutlined, CloseCircleOutlined, DeleteOutlined, DownloadOutlined, UnorderedListOutlined } from '@ant-design/icons';
...
function getDateRange(tab: RangeTab): { minDate: string; maxDate: string } | null {
...
export default function ReportsPage() {
    ...
    const dateRange = useMemo(() => getDateRange(activeRangeTab), [activeRangeTab]);

    const { data: reportsResponse, isLoading } = useGetReportsQuery({
        page: currentPage,
        limit: pageSize,
        ...(activeStatusTab === 'APPROVED' && dateRange && {
    ...
    const handleApprove = async (id: string) => {
        try {
            const result = await updateReportStatus({ id, status: 'APPROVED' }).unwrap();
            if (result.success) {
    ...
    const handleReject = async (id: string) => {
        try {
            const result = await updateReportStatus({ id, status: 'REJECTED' }).unwrap();
            if (result.success) {
    ...
    const handleDelete = async (id: string) => {
        try {
            const result = await updateReportStatus({ id, status: 'DELETED' }).unwrap();
            if (result.success) {
        ...
        try {
            ...
            for (let page = 1; page <= totalPages; page++) {
                const params = new URLSearchParams({
                    page: String(page),
                    limit: '100',
                    ...(dateRange && { minDate: dateRange.minDate, maxDate: dateRange.maxDate }),
                    status: 'APPROVED',
                });
            ...
            const sheetName = activeRangeTab === 'weekly' ? 'Weekly Report' : activeRangeTab === 'monthly' ? 'Monthly Report' : 'All Reports';
            ...
            const colWidths = Object.keys(excelData[0]).map((key) => ({
                wch: Math.max(
                    key.length,
                    ...excelData.map((row: Record<string, unknown>) => String(row[key]).length)
                ) + 2,
            }));
            ...
            toast.success(`${sheetName} downloaded successfully! (${allReports.length} records)`);
        } catch {
            ...
            <div className="pt-6 md:pt-2">
                ...
                <div className="mb-6">
                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-5">
                        <div>
                            <h2 className="text-2xl font-bold text-gray-900 dark:text-white tracking-tight transition-colors">Transaction Reports</h2>
                            <p className="text-gray-500 dark:text-gray-400 mt-1 text-sm transition-colors">View and download stock transaction reports</p>
                        </div>
                        {activeStatusTab === 'APPROVED' && (
                            ...
                            >
                                Download {activeRangeTab === 'weekly' ? 'Weekly' : activeRangeTab === 'monthly' ? 'Monthly' : 'All'} Report
                            </Button>
                        )}
                    </div>
                        ...
                        <table className="min-w-4xl xl:min-w-full divide-y divide-gray-200 dark:divide-[#303030]">
                            ...
                            <tbody className="bg-white dark:bg-[#141414] divide-y divide-gray-200 dark:divide-[#303030] transition-colors">
                                {isLoading ? (
                                    ...
                                    reports.map((report) => (
                                        <tr key={report.id} className="hover:bg-gray-50 dark:hover:bg-[#1f1f1f] transition-colors">
                                            {/* Product */}
                                            ...
                                            {/* Total Sale */}
                                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600 dark:text-gray-400 transition-colors">
                                                {report.totalSalePrice > 0 ? formatCurrency(report.totalSalePrice) : '—'}
                                            </td>

                                            {/* Profit/Loss */}
                                            <td className="px-6 py-4 whitespace-nowrap text-sm font-medium transition-colors">
                                                <span className={report.profitLoss > 0
                    ...
                    {!isLoading && reports.length === 0 && (
                        <div className="text-center py-12">
                            <div className="text-gray-500 dark:text-gray-400 text-lg mb-2 transition-colors">
                                {activeStatusTab === 'PENDING' ? 'No pending reports' : activeStatusTab === 'REJECTED' ? 'No rejected reports' : 'No transactions found'}
                            </div>
